...
export function SelectAlbumDialog(props: SelectAlbumDialogProps) {
    ...
    return (
        <Dialog open={open} onOpenChange={rest.onOpenChange}>
            <DialogContent>
                ...
                <SelectAlbumDialogBody {...rest} />
            </DialogContent>
        </Dialog>
    );
}

function SelectAlbumDialogBody({ onOpenChange, onSuccess }: Omit<SelectAlbumDialogProps, "open">) {
                ...
                {isFetching ? (
                    Array(3)
                        ...
                        .map((_, i) => (
                            <div
                                key={i}
                                className="w-full h-14 bg-foreground/5 border-b border-input/30 animate-pulse delay-(--loading-delay)"
                                style={{ "--loading-delay": `${i * 0.2}s` } as React.CSSProperties}
                            ></div>
                        ))
                ) : albums.length !== 0 ? (
                    albums.map(p => (
                        <Button
                            ...
                            onDoubleClick={() => navigateToAlbum(p)}
                        >
