...
import { getOriginalPath, getThumbPath, QUICK_EASE } from "@/lib/utils";
    ...
    return (
        <div className={cn("flex flex-col absolute inset-0 z-50 transition-colors duration-200", !item ? "bg-transparent pointer-events-none" : "bg-background")}>
            <TransformWrapper centerOnInit disablePadding>
                ...
                >
                    {item && (
                        <motion.div className="size-full flex justify-center items-center *:size-full" layoutId={`item-${item.id}`} transition={{ duration: 0.6, ease: QUICK_EASE }}>
                            <img src={getThumbPath(item.id, selectedLibrary?.path)} className={!loaded ? "block" : "hidden"} />
                            <img src={getOriginalPath(item, selectedLibrary?.path)} className={loaded ? "block" : "hidden"} onLoad={() => setLoaded(true)} />
                        </motion.div>
                    )}
                </TransformComponent>
            </TransformWrapper>
