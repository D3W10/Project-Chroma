...
export function DialogPaged({ pages, defaultPage, open, onOpenChange, onPageSwitch }: DialogPagedProps) {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent
                className={cn("overflow-hidden", !dialogMoving ? "duration-350 delay-25" : "")}
                showCloseButton={pages[dialogPage].closeable ?? true}
                style={{ height: `${dialogHeight}px` }}
                onAnimationStart={() => !open && setDialogMoving(true)}
                onAnimationEnd={() => open && setTimeout(() => setDialogMoving(false), 100)}
            >
                <DialogPagedContext.Provider value={{ page: dialogPage, setPage, close: () => onOpenChange(false) }}>
                    <AnimatePresence initial={false} mode="popLayout" custom={dialogDirection}>
                        {Object.keys(pages).map(
                            page =>
                                dialogPage === page && (
                                    <animate.div key={page} variants={slideVariants} custom={dialogDirection} initial="initial" animate="target" exit="exit" className="flex flex-col gap-6">
